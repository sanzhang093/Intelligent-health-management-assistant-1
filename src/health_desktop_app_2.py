...
"""
智能健康管理桌面应用 (简化版)
基于tkinter构建的桌面GUI应用，兼容症状问诊Agent不可用的情况
...
class HealthDesktopApp2:
    ...
    def __init__(self):
        ...
        self.symptom_agent_available = False
    ...
    def create_header(self, parent):
        ...
        self.agent_status_label.pack(side=tk.RIGHT, padx=(0, 10))
    ...
    def create_left_panel(self, parent):
        ...
        # 重新初始化按钮
        self.reinit_btn = ttk.Button(
            left_frame,
            text="🔄 重新初始化",
            command=self.reinitialize_system
        )
        self.reinit_btn.pack(fill=tk.X, pady=2)
    ...
    def create_chat_area(self, parent):
        ...
        self.chat_text.tag_configure("warning", foreground="#ff9800", font=("Arial", 9, "bold"))
    ...
    def initialize_controller(self):
        ...
        def init_thread():
            try:
                ...
                # 检查症状问诊Agent是否可用
                try:
                    symptom_agent = self.controller._init_symptom_agent()
                    self.symptom_agent_available = symptom_agent is not None
                except:
                    self.symptom_agent_available = False
                ...
                self.root.after(0, self.update_agent_status)
                
                if self.symptom_agent_available:
                    self.root.after(0, lambda: self.update_status("✅ 系统初始化完成"))
                else:
                    self.root.after(0, lambda: self.update_status("⚠️ 系统初始化完成（症状问诊Agent不可用）"))
                
            except Exception as e:
                self.root.after(0, lambda: self.update_status(f"❌ 初始化失败: {str(e)}"))
                self.root.after(0, lambda: messagebox.showerror("初始化错误", f"系统初始化失败:\n{str(e)}"))
...
控制器: 已初始化
症状问诊Agent: {'可用' if self.symptom_agent_available else '不可用'}
健康管理Agent: 可用
时间: {datetime.now().strftime('%H:%M:%S')}"""
    ...
    def update_agent_status(self):
        """更新Agent状态"""
        if self.symptom_agent_available:
            self.agent_status_label.config(text="✅ 症状问诊Agent: 可用", foreground="green")
        else:
            self.agent_status_label.config(text="⚠️ 症状问诊Agent: 不可用", foreground="orange")
    ...
    def reinitialize_system(self):
        """重新初始化系统"""
        if messagebox.askyesno("确认", "确定要重新初始化系统吗？"):
            self.controller = None
            self.available_users = []
            self.symptom_agent_available = False
            self.initialize_controller()
...
"""
                ...
                if question_type == "自动判断":
                    ...
                    if self.symptom_agent_available:
                        ...
                        if agent:
                            ...
                            # 降级处理
                            result = self.fallback_to_health_agent(question, "症状问诊Agent初始化失败")
                    else:
                        # 降级处理
                        result = self.fallback_to_health_agent(question, "症状问诊Agent不可用")
                else:  # 健康管理
                    health_agent = self.controller._init_health_agent()
    ...
    def fallback_to_health_agent(self, question, reason):
        """降级到健康管理Agent处理"""
        health_agent = self.controller._init_health_agent()
        health_agent.set_current_user(self.current_user)
        agent_result = health_agent.analyze_health_query(question)
        return {
            'category': f'症状问诊(由健康管理Agent处理)',
            'classification_confidence': 0.8,
            'classification_reason': reason + '，使用健康管理Agent',
            'agent_result': agent_result,
            'timestamp': datetime.now().isoformat()
        }
...
{answer}"""
            ...
            # 如果是降级处理，添加警告信息
            if "由健康管理Agent处理" in category:
                self.add_message_to_chat("system", f"⚠️ 注意: {reason}")
    ...
    def add_message_to_chat(self, role, content):
        ...
        if role == "user":
            ...
            self.chat_text.insert(tk.END, "💻 系统: ", "warning")
