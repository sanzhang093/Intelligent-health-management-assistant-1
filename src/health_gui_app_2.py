...
class HealthGUIApp2:
    ...
    def __init__(self):
        ...
        if 'symptom_agent_available' not in st.session_state:
            st.session_state.symptom_agent_available = False
            ...
            try:
                with st.spinner("🔄 正在初始化智能健康管理系统..."):
                    ...
                    try:
                        ...
                        st.session_state.symptom_agent_available = symptom_agent is not None
                    except:
                        st.session_state.symptom_agent_available = False
                ...
                # 显示Agent状态
                if st.session_state.symptom_agent_available:
                    st.success("✅ 症状问诊Agent可用")
                else:
                    st.warning("⚠️ 症状问诊Agent不可用，将使用健康管理Agent处理所有问题")
        ...
        if st.session_state.controller_initialized:
            if st.session_state.symptom_agent_available:
                ...
                <div class="success-box">
                    <h4>✅ 系统状态正常</h4>
                    <ul>
                        <li><strong>症状问诊Agent</strong>：可用</li>
                        <li><strong>健康管理Agent</strong>：可用</li>
                        <li><strong>智能分类</strong>：正常工作</li>
                    </ul>
                </div>
                """, unsafe_allow_html=True)
            else:
                ...
                <div class="warning-box">
                    <h4>⚠️ 系统状态：部分功能受限</h4>
                    <ul>
                        <li><strong>症状问诊Agent</strong>：不可用（网络问题）</li>
                        <li><strong>健康管理Agent</strong>：可用</li>
                        <li><strong>智能分类</strong>：正常工作，症状问诊问题将由健康管理Agent处理</li>
                    </ul>
                </div>
                """, unsafe_allow_html=True)
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            st.markdown("### 🤖 Agent状态")
            if st.session_state.symptom_agent_available:
                st.success("✅ 症状问诊Agent: 可用")
            else:
                st.warning("⚠️ 症状问诊Agent: 不可用")
            st.success("✅ 健康管理Agent: 可用")
    ...
    def render_chat_interface(self):
        ...
        if submit_btn and user_question.strip():
            if not st.session_state.current_user:
                ...
                self.process_user_question(user_question.strip(), question_type)
    
    def process_user_question(self, question: str, question_type: str = "自动判断"):
        """处理用户问题"""
        try:
            # 添加用户消息到历史
            ...
            with st.spinner("🤖 正在分析您的问题..."):
                if question_type == "自动判断":
                    ...
                    if st.session_state.symptom_agent_available:
                        ...
                        if agent:
                            agent.set_current_user(st.session_state.current_user)
                            agent_result = agent.analyze_health_query(question, st.session_state.current_user)
                            result = {
                                'category': '症状问诊',
